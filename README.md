...
面向 Codex 的“预生成 PPT 设计风格 image 批量重建为可编辑 PowerPoint”下游生产工作流。

上游 PPT 项目负责生成已经确定视觉设计、版式和内容结构的逐页 image；本项目承接这些 image，完成从“设计预览”到“可编辑 PPTX 成品”的下游转换。它会严格按照页码顺序，一次只处理一张 image；当前页完成 artifact 预览、真实 PowerPoint 预览、结构、元素清单和页面边界检查后，才继续下一页。任务中断后可以从已保存的队列状态恢复，全部完成后会生成审计结果和交付包，可按顺序合并为一个 PPTX，并清理临时 hook。

```text
上游：PPT 内容与设计生成 → 预生成 PPT 设计风格 image
                                      ↓
本项目（下游）：image 预览 → 可编辑单页 PPTX → QA → 合并 PPTX 成品
```

## 适合什么场景

- 把一组预生成 PPT 设计风格 image 重新制作成可编辑 PPTX
- 将标题、正文、表格、箭头、边框和色块恢复为可编辑对象
- 把照片、设备渲染、软件界面和复杂图标裁成独立图片
...
- `codeximage-to-editable-ppt-v1`：负责单页 image 的精修可编辑重建。
...
上游应先输出已经完成 PPT 视觉设计的逐页 image。建议把所有 image 放在同一个文件夹中，并按页码命名：
...
skill 必须先收到用户提供的真实 image 地址，才会创建 hook。它不会通过全盘搜索猜测输入目录。
...
- 不允许简单地把整张 image 铺满页面并宣称已经可编辑。

## 常见问题

### Codex 要求先提供 image 地址

这是正常的输入安全规则。请提供 image 文件夹或某张编号 image 的绝对路径。
...
每页都会经历拆解、重建、渲染和检查。复杂图表、软件界面和设备渲染较多时，需要更长时间。
